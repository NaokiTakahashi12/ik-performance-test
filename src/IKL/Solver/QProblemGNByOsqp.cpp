...
#include <IKL/Solver/QProblemGNByOsqp.hpp>
...
namespace IKL {
    namespace Solver {
        ...
        QProblemGNByOsqp<Scaler>::QProblemGNByOsqp(const Parameters<Scaler> &param) : NumericIK<Scaler>(param) {
            this->solver_name = "QProblemGNByOsqp";
        ...
        template <typename Scaler>
        QProblemGNByOsqp<Scaler>::~QProblemGNByOsqp() {
        }

        template <typename Scaler>
        Math::VectorN<Scaler> &QProblemGNByOsqp<Scaler>::update_delta_step(const Math::MatrixNxN<Scaler> &jacobian, const Math::VectorN<Scaler> &error, const Kinematic::JointState<Scaler> &joint_state) {
            ...
            const Math::MatrixNxN<Scaler> mP = jacobian.transpose() * weight_e * jacobian;
        ...
        template <typename Scaler>
        void QProblemGNByOsqp<Scaler>::solve_setting(const Kinematic::JointState<Scaler> &init_joint_state) {
        ...
        template class QProblemGNByOsqp<double>;
    }
}
