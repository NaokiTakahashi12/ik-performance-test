...
#include <IKL/Solver/QProblemGNByqpOASES.hpp>
...
namespace IKL {
    namespace Solver {
        ...
        QProblemGNByqpOASES<Scaler>::QProblemGNByqpOASES(const Parameters<Scaler> &param) : NumericIK<Scaler>(param) {
            this->solver_name = "QProblemGNByqpOASES";
            solve_number_of_variables = 0;
        }

        template <typename Scaler>
        QProblemGNByqpOASES<Scaler>::~QProblemGNByqpOASES() {
        }

        template <typename Scaler>
        Math::VectorN<Scaler> &QProblemGNByqpOASES<Scaler>::update_delta_step(const Math::MatrixNxN<Scaler> &jacobian, const Math::VectorN<Scaler> &error, const Kinematic::JointState<Scaler> &joint_state) {
            ...
            H = jacobian.transpose() * weight_e * jacobian;
        ...
        template <typename Scaler>
        void QProblemGNByqpOASES<Scaler>::solve_setting(const Kinematic::JointState<Scaler> &init_joint_state) {
        ...
        template class QProblemGNByqpOASES<double>;
    }
}
